//
//  PreferenceCountValidator.swift
//  Pods
//
//  Created by Zvonimir Medak on 20.03.2025..
//
...
///
public struct PreferenceCountValidator<T> where T: Collection {
...
extension PreferenceCountValidator: PreferenceValidator {
    ...
    public func validate(value: T) -> Bool {
        (min ?? Int.zero) <= value.count && value.count <= (max ?? Int.max)
    }
}
